...
struct ModbusDeviceCommand {
  ...
  ModbusDevice *device;  // Device that queued this command (nullptr = broadcast to all matching)

  ModbusDeviceCommand(const uint8_t *src, uint16_t len, ModbusDevice *device = nullptr)
      : data(std::make_unique<uint8_t[]>(len + 2)), size(len + 2), device(device) {
...
class Modbus : public uart::UARTDevice, public Component {
 public:
  ...
  void send(uint8_t address, uint8_t function_code, uint16_t start_address, uint16_t number_of_entities,
            uint8_t payload_len = 0, const uint8_t *payload = nullptr, ModbusDevice *device = nullptr);
  void send_raw(const std::vector<uint8_t> &payload, ModbusDevice *device = nullptr);
 ...
 protected:
  ...
  void queue_raw_(const uint8_t *data, uint16_t len, ModbusDevice *device = nullptr);
  ...
  ModbusDevice *current_requester_{nullptr};
